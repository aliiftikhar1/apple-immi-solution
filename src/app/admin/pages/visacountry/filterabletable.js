...
const FilterableTable = ({ data, userRole, userBranch, country, userId, filetypedata, fetchData }) => {
  ...
  useEffect(() => {
    ...
    if (userRole === 'employee') {
      ...
      fetch(`/api/visacountrymanager?userId=${userId}&country=${country}`)
        .then(response => response.json())
        .then(branchUsers => {
          setFilteredData(branchUsers.customer || []); // Ensure it's an array
        })
        .catch(error => {
          console.error('Error fetching branch users:', error);
          setFilteredData([]); // Set empty array on error
        });
    } else {
      setFilteredData(
  ...
  }, [filter, data, filetypes, userRole, userId, userBranch, country]);
  ...
  const handleEditItem = async (item) => {
    ...
    try {
      ...
      const result = await response.json();
      if (response.ok) {
        setImages(result.map(img => ({ base64: `https://appstore.store2u.ca/uploads/${img.imgurl}`, type: img.type })));
      } else {
  ...
  const previewimageurl = previewurl || (`https://appstore.store2u.ca/uploads/${currentItem?.imgurl}`);
                  ...
                  <div>
                    ...
                    <div className="flex w-full flex-row">
                      ...
                      <div className="ml-7 mx-auto flex justify-center items-center">
                        <img src={previewimageurl} alt='image' className='w-[200px] h-[200px] rounded-xl'/>
                      </div>
                    </div>
                  </div>
